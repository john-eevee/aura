defmodule AuraWeb.ClientLive.Form do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.form for={@form} id="client-form" phx-change="validate" phx-submit="save">
        ...
        <.input field={@form[:since]} type="datetime-local" label="Since" />
        <.input
          field={@form[:status]}
          type="select"
          label="Status"
        ...
        />
        <.input field={@form[:industry_type]} type="text" label="Industry type" />
        <footer>
          <.button phx-disable-with="Saving..." variant="primary">Save Client</.button>
          <.button navigate={return_path(@current_scope, @return_to, @client)}>Cancel</.button>
        </footer>
      </.form>
    </Layouts.app>
    """
  ...
  def handle_event("validate", %{"client" => client_params}, socket) do
    changeset = Clients.change_client(socket.assigns.current_scope, socket.assigns.client, client_params)
